...
export function parseLimit(raw) {
    ...
    if (!Number.isFinite(parsed) || !Number.isInteger(parsed)) {
        throw new ArgumentError(`--limit must be an integer between 1 and 1000, got ${JSON.stringify(raw)}`);
    }
    if (parsed < 1 || parsed > 1000) {
        throw new ArgumentError(`--limit must be between 1 and 1000, got ${parsed}`);
    }
    return parsed;
}
/**
