...
cli({
  site: 'kickstarter',
  name: 'search',
  description: 'Search Kickstarter projects (JSON-friendly fields for integrations)',
  domain: DOMAIN,
  strategy: Strategy.COOKIE,
  args: [
    { name: 'query_or_url', positional: true, required: false, help: 'Search keyword or full Kickstarter discover URL' },
  ...
  func: async (page, kwargs) => {
    ...
    if (payload?.authRequired && items.length === 0) {
      throw new AuthRequiredError(DOMAIN, 'AuthRequired: kickstarter/search requires login or passed verification');
    }

    if (!items.length) {
      throw new EmptyResultError('kickstarter/search', 'No project records found (blocked page or selector mismatch)');
    }
